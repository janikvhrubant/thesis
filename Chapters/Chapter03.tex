...
\begin{equation}
    ...
    V_{\mathrm{HK}}(f) = \sum_{u\subset \{1,\dots,s\}} V_{[\boldsymbol{a}^{-u},\boldsymbol{b}^{-u}]}\big(f(x^{-u};\boldsymbol{b}^u)\big).
\end{equation}
...
with bounded mixed derivatives and its Hardy--Krause variation is finite. This
can easily be verified using equation \eqref{eq:variation_hardy_krause_bound}
which will be introduced in Chapter~\ref{chapter6}.
\end{example}

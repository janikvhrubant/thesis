...
$f\in C^s([0,1]^s, \mathcal{X})$ and $\eta \in C^s(\mathcal{X})$. For $\eta
\circ f$ this yields
\begin{equation*}
    V_{[0,1]^s}(\eta \circ f) \leq \int\limits_{[0,1]^s} \bigg| \frac{\partial^s}{\partial x_1 \cdots \partial x_s} (\eta \circ f)(x) \bigg| \, dx.
\end{equation*}
By applying the multivariate Faà di Bruno formula from
Proposition~\ref{prop:multivariate_faa_di_bruno} it follows that
\begin{equation*}
    V_{[0,1]^s}(\eta \circ f) \leq \int\limits_{[0,1]^s} \bigg| \sum_{\pi} \eta^{(|\pi|)}\circ f(x) \cdot \prod_{B\in\pi} \frac{\partial^{|B|}}{\prod_{j\in B}\partial x_j}f(x) \bigg| \, dx.
\end{equation*}
As for a finite dimension $s$ the number of partitions $\pi$ is finite, the
variation in the sense of Vitali is bounded, if for each partition $\pi$ of
$\{1,\dots,s\}$ the following holds:
\begin{equation*}
    \int\limits_{[0,1]^s} \bigg| \eta^{(|\pi|)}\circ f(x) \cdot \prod_{B\in\pi} \frac{\partial^{|B|}}{\prod_{j\in B}\partial x_j}f(x) \bigg| \, dx < \infty.
\end{equation*}
As $\eta \in C^s(\mathcal{X})$ is continuous, the composition
$\eta \circ f$ is also continuous. Therefore, to show that the variation in the sense of Vitali is bounded, it sufficies to show the following for all partitions $\pi$ of the set $\{1,\ldots,s\}$:
\begin{equation*}
    \int\limits_{[0,1]^s} \prod_{B\in\pi} \bigg| \frac{\partial^{|B|}}{\prod_{j\in B}\partial x_j}f(x) \bigg| \, dx < \infty.
\end{equation*}
\end{remark}
...
\begin{proof}[Proof (Theorem~\ref{thm:main-theorem})] \ \\
    ...
    The ground truthh function $f$ fullfils this by assumptions. As the

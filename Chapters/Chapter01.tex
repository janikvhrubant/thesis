...
may be expensive or impractical to evaluate analytically. Throughout this
thesis, we will limit our discussion on functions of the type $f: \mathbb{R}^s
\to \mathbb{R}$.
...
% \paragraph{High-Dimensional Integration in Applications}\ \\
...
% \begin{itemize}
%     \item \textbf{Bayesian statistics}: Computing posterior expectations,
%     marginal likelihoods, or predictive distributions.
%     \item \textbf{Financial mathematics}: Pricing complex financial derivatives
%     and evaluating risk measures under stochastic models.
%     \item \textbf{Computational imaging}: Solving high-dimensional integrals in
%     light transport for computer graphics, atmospheric radiative transfer in
%     remote sensing and, more recently, photon transport simulation in medical
%     imaging.
...
% The remainder of this chapter explores how \ac{mc} and \ac{qmc} methods
% address these challenges. 

% Before getting into \acl{qmc}, we briefly review the \acl{mc} method and its
% fundamental convergence properties in the next section.
...
% Whereas classical \ac{mc} methods approximate integrals using randomly sampled
% points, \ac{qmc} methods replace stochasticity with a deterministic strategy,
% aiming to cover the integration domain in a more uniform and structured way.
% This shift allows \ac{qmc} methods to achieve faster convergence rates under
% certain conditions, particularly for smooth functions or problems with low
% effective dimension.

% Without a rigorous definition of construction of \ac{qmc} point sets this
% section is meant to underline the key differences between \ac{mc} and \ac{qmc}
% methods to motivate the use of deterministic sampling strategies. The formal
% definition of \ac{qmc} point sets and their properties will be introduced in
...
% To motivate the use of deterministic \ac{qmc} methods, we first define the
% \emph{quasi-Monte Carlo estimator} for an integrand $f$ over the unit cube
% $[0,1]^s$ and a \ac{qmc} point set $\{\boldsymbol{x}_n\}_{n=1}^{N}
% \subset [0,1]^s$.



% The integration (approximation) functions $I$, $I_N^{\mathrm{MC}}$ and
% $I_N^{\mathrm{QMC}}$ will be further used without explicit reference to the
% integrand $f$ when the context is clear.

% Unlike the randomly sampled points in \acl{mc} methods, the deterministically
% constructed points in \acl{qmc} methods are typically drawn to fill the
% integration domain more evenly and avoid clustering. This often leads to lower
% integration errors, especially for smooth functions or problems.
...
% A straight forward alternative to achieve more uniform distribution as in
% \ac{mc} sampling is the use of regular tensor-product grids. However, these
% grids suffer from the \emph{curse of dimensionality}: placing $m$ points per
% dimension leads to a total of $N = m^s$ points, which becomes infeasible even
% for moderate $s$. Furthermore, grids are not easily extensible -- adding new
...
% By contrast, many \ac{qmc} sequences, such as the Sobol' and Halton Sequences as
...
% algorithms that might increment the number of samples needed over time. This is
% often applied by extending the training set of a neural network with new samples
% or by adding more photons in \ac{ct} simulations. In these cases, the point set
% can grow dynamically without requiring a complete recomputation of the existing
% points. However, not all \ac{qmc} constructions share this property such as
% lattice rules and some optimized designs are fixed-size by nature.

% This shift from probabilistic sampling to deterministic \ac{qmc} sequences also
% changes the way we analyze errors: instead of using statistical bounds, we rely
% on \emph{discrepancy measures}, which quantify the uniformity of the point set.
% These concepts, along with the notion of function variation, will be introduced
...
% % ------------------------------------------------------------------------------
% \subsection{Monte Carlo vs. Quasi-Monte Carlo}
% \label{subsec:mc-vs-qmc}
% % ------------------------------------------------------------------------------

% \ac{mc} and \ac{qmc} methods can be applied to the same high-level goal:
% estimating an integral by averaging function evaluations at selected sample
% points. The difference lies in the sampling strategy -- stochastic versus
% deterministic -- and the consequences this has for accuracy, convergence and

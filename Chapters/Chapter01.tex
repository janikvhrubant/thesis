...
focuses on the training of neural networks using \ac{qmc} methods, while
Part~\ref{part3} applies these techniques to photon transport simulation in
\acf{ct}.
...
integration based on averaging function evaluations at randomly sampled points.
Its convergence rate of $O(N^{-N/2})$ is independent of the dimension,
making it particularly attractive for high-dimensional problems.
...
Let $f \in L^1([0,1]^s)$. Then, the Monte Carlo Estimator from \eqref{eq:mc-estimator} converges almost surely to the integral $I(f)$ from \eqref{eq:integration_problem} as the number of samples $N$ tends to infinity:
\begin{equation}
    \mathbb{P}\!\left( \lim_{N \to \infty} I_N^{\mathrm{MC}}(f) = 
    \int_{[0,1]^s} f(\mathbf{x})\, \mathrm{d}\mathbf{x} \right) = 1,
\end{equation}
i.e., $I_N^{\mathrm{MC}}(f) \xrightarrow{\mathrm{a.s.}} \int_{[0,1]^s}

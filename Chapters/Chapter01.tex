...
The first part of this thesis is structured into three chapters. Chapter~\ref{chapter1} introduces the consepts of \ac{mc} and \ac{qmc} methods to estimate the integral from \eqref{eq:integration_problem}. Chapter~\ref{chapter2} provides a formal introduction to discrepancy theory and some constructions of low-discrepancy sequences. In Chapter~\ref{chapter3}, the concepts of variation and error bounds as the \emph{Koksma-Hlawka inequality} are introduced. The subsequent Part~\ref{part2} of this thesis
focuses on the application of \ac{qmc} methods for training of neural networks, while Part~\ref{part3} applies \ac{qmc} techniques to photon transport simulation in \acl{ct}.
...
Using independence and linearity of expectation, the variance of
$I_N^{\mathrm{MC}}(f)$ is
\begin{equation}
    \mathrm{Var}[I_N^{\mathrm{MC}}(f)] = \frac{\mathrm{Var}[f]}{N}.
\end{equation}
Applying Jensen's inequality yields the stated bound on the expected absolute
error.
\end{proof}
...
Hereby $[\boldsymbol{a},\boldsymbol{b})$ denotes the half-open interval in

...
constructions of low-discrepancy sequences such as Sobol' and Halton. Together
with the concepts of variation end error bounds introduced in
Chapter~\ref{chapter3}, these form the theoretical backbone for the applications
of \ac{qmc} methods in the subsequent parts of this thesis: Part~\ref{part2}
focuses on using low-discrepancy sequences for training of neural networks,
while Part~\ref{part3} is applying \ac{QMC} techniques to photon transport
simulation in \acf{ct}.
...
accuracy, computational cost and robustness with respect to the structure of the
integrand.
...
identically distributed drawn random samples. Then Monte Carlo estimator of the
integral $I$ from \eqref{eq:integration_problem} is given by
\begin{equation}
...
This estimator is unbiased and converges almost surely to the exact value of the
integral, as established by the Strong Law of Large Numbers:

\begin{theorem}[Strong Law of Large Numbers] \ \\
Let $f \in L^1([0,1]^s)$. Then, the Monte Carlo Estimator from
\eqref{eq:mc-estimator} converges almost surely to the integral $I(f)$ from
\eqref{eq:integration_problem} as the number of samples $N$ tends to infinity:
\begin{equation}
...
\ac{mc} estimator has a well-defined convergence rate.

...
The applied fiter in Figure~\ref{fig:spectrum100kvp_with_filter} effectively
...
The photon energy is sampled according to the X-ray tube spectrum as described
in Section~\ref{sec:xray_tube_spectrum}. As in
...
\begin{enumerate}[label=(\roman*)]
    \item Construct the appropriate probability density function from the given spectrum.
    \item Compute the \ac{cdf} from the probability density function $F$.
    \item Determine the generalized inverse of the \ac{cdf} $F^{-1}$.
    \item Sample a uniformly distributed random variable $U$ on the interval $[0,1]$ and compute the photon energy as $E = F^{-1}(U)$.
\end{enumerate}
...
% Inverse transform sampling \cite{muller2012monte} is a method to generate random
% samples from a target distribution using uniformly distributed random variables,
% such as those generated by a Quasi-Monte Carlo sequence.
...
\begin{theorem} \ \\
Let $F:\mathbb{R}\to[0,1]$ be a continuous and strictly increasing cumulative
distribution function and $F^{-1}$ its generalized inverse. Then for a random
variable $X$ generated by
\begin{equation*}
    X := F^{-1}(U)
\end{equation*}
...
because $U \sim \mathcal{U}(0,1)$ and thus  $\mathbb{P}[U \leq u] = u$ for all $u \in [0,1]$. This shows that $X$ has CDF $F$.
\end{proof}
...
must be specified. Within the simulation framework, two different source models
are considered:
...
pointlike focal spot and divergent beam. In line with common practice (e.g.\
...
$\vec{p}=(p_1,p_2,p_3)\in\mathbb{R}^3$. To describe the corresponding
probability measure, recall that the unit sphere $S^2$ in spherical coordinates
is parameterized by
\[
...
\]
see for instance \cite[Theorem~5-6]{spivak2018calculus}. A uniform distribution
on the spherical surface thus corresponds to a constant density with respect to
this measure. Restricting the support to the spherical cap
...
with the paramters listed in Table~\ref{tab:xray_params}.
...
matter. These processes depend on the photon energy and the atomic molecules in
the material respecting the density and molecular distribution of the material.
A visualization of these interaction mechanisms is shown in
...
\si{\per\centi\metre}, applied within the Beer--Lambert law, is obtained by
multiplying the mass attenuation coefficient $\tilde{\mu}$ with the density
$\rho$ of the material:
...
within the phantom. Therefore, a simple helper function is returning the
\emph{linear attenuation coefficient} $\mu$ for a given position $x$ and energy
$E$. The coefficients are summarized in
...
Database} \cite{nistXCOM}.

When an X-ray photon two main effects are considered:
\begin{itemize}
...
The probability, that a phantom survives from a certain point $A$ to another point $B$ without any interaction, is described by as follows \cite{bezug2014computed}:

$$P(A \to B) = \exp\bigg[-\int_{A}^{B}{A + \mu\bigg(\frac{s}{\|\overrightarrow{AB}\|}(B-A),E \bigg)\,ds}\bigg]$$

To sample the free path length of a photon, one can project the photon within a given direction $\vec{\omega}$ from its current position $A$ to the detector position $\hat{C}$, as shown in Figure~\ref{fig:exit_point}. Then, a random variable $U \sim \mathcal{U}(0,1)$ is sampled.
...
For readability, $k=\tfrac{E}{m_ec^2}$ is used in the following.
Here, $m_ec^2$ denotes the electron rest energy which is approximately $0.511$ and $r_e$ denotes the classical
electron radius. This yields the following form of the Klein-Nishina formula:
...
to employ a simpler envelope distribution $g(x)$, from which samples can be
drawn efficiently, and which dominates $f(x)$ up to a constant factor. More
precisely, one requires a constant $0< c \leq 1$ such that $c\,f(x) \leq g(x)$
for all $x$. A candidate value $x$ is drawn from $g(x)$, and an auxiliary
uniform random number $u \sim \mathcal{U}(0,1)$ is generated. The sample $x$ is
accepted if $u \leq \tfrac{c\,f(x)}{g(x)}$, otherwise it is rejected and the
procedure is repeated until an acceptance occurs.

The efficiency of the algorithm depends on how tightly $g(x)$ bounds $c\,f(x)$:
...
and allows efficient evaluation. For more details, see for instance \cite[Chap 4]{muller2012monte} or \cite[Chap. 8.1]{leobacher2014introduction}.
...
\frac{a(k)}{b(k)+1}=\zeta \quad&\Rightarrow\quad \frac{2(b(k)-1)}{b(k)+1}=\zeta.
\end{align*}
Solving for $b(k)$ by multiplying out gives
$$
...
$h(k,x)>0$ because $b(k)>1$ implies $b(k)-x>0$ on $[-1,1]$, we have $R_k(x)>0$
...
Now the acceptance--rejection method can be applied. To sample the scattering angle $\theta$, $x=\cos\theta$ is sampled from the distribution $h$ the inverse
transform method:

The cumulative distribution function of $h$ is
$$
H(x) = \int_{-1}^x \frac{a}{b-t}\,dt
...
This values is accepted if $\frac{f(k,x)}{h(k,x)} \geq u_1$, where $u_1$ denotes a random input variable. If the value is not accepted, a new value is sampled until acceptance occurs. The scattering angle $\theta$ is then obtained by $\theta = \arccos(x)$

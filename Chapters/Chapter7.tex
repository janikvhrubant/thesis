...
\State $\vec{w} \gets \vec{d} / \|\vec{d}\|$ \Comment{Normalized cone axis}
\If{$|w_3| < 0.999$}
    \State $\vec{a} \gets (0, 0, 1)$
\Else
    \State $\vec{a} \gets (1, 0, 0)$
\EndIf
\State $\vec{u} \gets \frac{\vec{a} \times \vec{w}}{\|\vec{a} \times \vec{w}\|}$ \Comment{Orthogonal vector}
\State $\vec{v} \gets \vec{w} \times \vec{u}$ \Comment{Complete right-handed basis}
